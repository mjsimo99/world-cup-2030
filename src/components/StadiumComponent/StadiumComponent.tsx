...
export default function StadiumComponent() {
    ...
    const getRandomImage = () => {
        const randomIndex = Math.floor(Math.random() * 10) + 1; // Adjust range based on available images
    ...
    return (
        ...
        <div>
            ...
            <h3 className="font-serif text-3xl text-[#343D33] capitalize md:text-4xl lg:text-5xl text-center mt-6 mb-6">Stadiums</h3>

            <div className="card flex justify-content-center flex-wrap w-auto" style={{ border: 'none' }}>
                {loading ? 'Loading...' : (error ? error :
                    stadiums.map((stadium, index) => (
                        <Card key={index} title={stadium.name} subTitle={`Capacity: ${stadium.capacity}, Location: ${stadium.location}`} header={<img alt="Card" src={getRandomImage()} />} className="md:w-25rem w-96 mx-4 mb-8">
                            <p className="m-0">
                                {stadium.city ? `City: ${stadium.city.name}` : ''}
                            </p>
                        </Card>
                    ))
                )}
